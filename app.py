...
# Switching to 1.5 Flash for higher quota stability
...
if prompt := st.chat_input("Partner, explain the 3rd Mountain..."):
    ...
    with st.chat_message("assistant"):
        try:
            # Direct generation via the high-quota stable path
            ...
            # AUTOMATIC HELICAL VISUALIZATION
            if any(x in prompt.lower() for x in ["hydrogen", "helical", "3rd mountain"]):
                t = np.linspace(0, 10, 500)
                fig = go.Figure(data=[go.Scatter3d(x=np.sin(t*5), y=np.cos(t*5), z=t, mode='lines', line=dict(color='#00f2ff', width=6))])
                fig.update_layout(title="Helical Path Visualization", scene=dict(bgcolor='black'), template="plotly_dark")
            ...
            if "429" in str(e):
                st.warning("⚠️ Research Quota Paused. Please wait 60 seconds for the next derivation.")
            else:
                st.error(f"Sync Jitter: {e}")
